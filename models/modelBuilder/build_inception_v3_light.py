...
def addInceptionT1(model, name , bottom,
    ...
    # 1x1
    model, top1 = addConvRelu(model, name='{}/p1_1x1'.format(name), bottom=bottom, num_output=p1_num_out,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
    # 1x1->3x3
    model, top2 = addConvRelu(model, name='{}/p2_1x1'.format(name), bottom=bottom, num_output=p2_1x1,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
    model, top2 = addConvRelu(model, name='{}/p2_3x3'.format(name), bottom=top2,   num_output=p2_num_out,
                                kernel_size=3, stride=1, pad=1, filler="xavier")
    # "5x5 factorized into 1x1->3x3->3x3
    model, top3 = addConvRelu(model, name='{}/p3_1x1'.format(name), bottom=bottom, num_output=p3_1x1,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
    model, top3 = addConvRelu(model, name='{}/p3_3x3a'.format(name), bottom=top3,  num_output=p3_num_out,
                                kernel_size=3, stride=1, pad=1, filler="xavier")
    model, top3 = addConvRelu(model, name='{}/p3_3x3b'.format(name), bottom=top3,  num_output=p3_num_out,
                                kernel_size=3, stride=1, pad=1, filler="xavier")
    # pool->[1x1]
    model, top4 = addPool(model, name='{}/p4_pool'.format(name), bottom=bottom,
                                kernel_size=3, stride=1, pad=1, pool_type=pool)
    model, top4 = addConvRelu(model, name='{}/p4_1x1'.format(name), bottom=top4, num_output=p4_num_out,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
    # concat
...
def addReductionR1(model, name , bottom,  p1_1x1, p1_3x3, p1_3x3s2, p2_3x3s2, pool):
    ...
    # p1:1x1->[3x3]->[3x3/2]
    model, top1 = addConvRelu(model, name='{}/p1_1x1'.format(name),  bottom=bottom, num_output=p1_1x1,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
    model, top1 = addConvRelu(model, name='{}/p1_3x3a'.format(name), bottom=top1,   num_output=p1_3x3,
                                kernel_size=3, stride=1, pad=1, filler="xavier")
    model, top1 = addConvRelu(model,name='{}/p1_3x3b'.format(name),  bottom=top1,   num_output=p1_3x3s2,
...
#   top2 = addConvBnRelu(model, name='{}/p2_1x1'.format(name), bottom=bottom, num_output=p2_1x1, kernel_size=1, stride=1, pad=0, filler="xavier")
    model, top2 = addConvRelu(model, name='{}/p2_3x3'.format(name), bottom=bottom,  num_output=p2_3x3s2,
                                kernel_size=3, stride=2, pad=0, filler="xavier")
...
def addInceptionT2(model, name , bottom,  num_proj, num_out, pool):
    # p1:1x1
    model, top1 = addConvRelu(model, name='{}/p1_1x1'.format(name), bottom=bottom, num_output=num_out,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
    # p2:1x1->1x7->7x1
    model, top2 = addConvRelu(model, name='{}/p2_1x1'.format(name), bottom=bottom, num_output=num_proj,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
    model, top2 = addConvRelu(model, name='{}/p2_1x7'.format(name), bottom=top2, num_output=num_proj,
                                kernel_h=1, kernel_w=7, pad_h=0, pad_w=3, stride=1, filler="xavier")
    model, top2 = addConvRelu(model, name='{}/p2_7x1'.format(name), bottom=top2, num_output=num_out,
                                kernel_h=7, kernel_w=1, pad_h=3, pad_w=0, stride=1, filler="xavier")

    # p3: 1x1->1x7->7x1->1x7->7x1
    model, top3 = addConvRelu(model, name='{}/p3_1x1'.format(name), bottom=bottom, num_output=num_proj,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
    model, top3 = addConvRelu(model, name='{}/p3_1x7a'.format(name), bottom=top3, num_output=num_proj,
                                kernel_h=1, kernel_w=7, pad_h=0, pad_w=3, stride=1, filler="xavier")
    model, top3 = addConvRelu(model, name='{}/p3_7x1a'.format(name), bottom=top3, num_output=num_proj,
                                kernel_h=7, kernel_w=1, pad_h=3, pad_w=0, stride=1, filler="xavier")
    model, top3 = addConvRelu(model, name='{}/p3_1x7b'.format(name), bottom=top3, num_output=num_proj,
                                kernel_h=1, kernel_w=7, pad_h=0, pad_w=3, stride=1, filler="xavier")
    model, top3 = addConvRelu(model, name='{}/p3_7x1b'.format(name), bottom=top3, num_output=num_out,
                                kernel_h=7, kernel_w=1, pad_h=3, pad_w=0, stride=1, filler="xavier")

    # pool->[1x1]
    model, top4 = addPool(model, name='{}/p4_pool'.format(name), bottom=bottom,
                                kernel_size=3, stride=1, pad=1, pool_type=pool)
    model, top4 = addConvRelu(model, name='{}/p4_1x1'.format(name), bottom=top4, num_output=num_out,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
    # concat
    tops = [top1, top2, top3, top4]
    model, top = addConcat(model, name='{}/concat'.format(name), bottoms=tops)
...
def addReductionR2(model, name , bottom,  num_proj, num_out, pool):
    ...
    # p1:[1x1]->[3x3/2]
    model, top1 = addConvRelu(model, name='{}/p1_1x1'.format(name), bottom=bottom, num_output=num_proj,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
    model, top1 = addConvRelu(model,name='{}/p1_3x3'.format(name), bottom=top1,  num_output=num_out,
                                kernel_size=3, stride=2, pad=0, filler="xavier")
    ...
    model, top2 = addConvRelu(model, name='{}/p2_1x7'.format(name), bottom=top2, num_output=num_proj,
                                kernel_h=1, kernel_w=7, pad_h=0, pad_w=3, stride=1,filler="xavier")
    model, top2 = addConvRelu(model, name='{}/p2_7x1'.format(name), bottom=top2, num_output=num_proj,
                                kernel_h=7, kernel_w=1, pad_h=3, pad_w=0, stride=1,filler="xavier")
    model, top2 = addConvRelu(model, name='{}/p2_3x3'.format(name), bottom=top2, num_output=num_proj,
                                kernel_size=3, stride=2, pad=0, filler="xavier")
    # p3: pool_3x3/2
    model, top3 = addPool(model, name='{}/p3_pool'.format(name), bottom=bottom,
                                kernel_size=3, stride=2, pad=0, pool_type=pool)
...
def addInceptionT3(model, name, bottom, p1_num_out, p2_num_out, p3_proj, p3_num_out, p4_num_out, pool):
    ...
    # p2:[1x1]->([1x3]+[3x1])
    model, top2 = addConvRelu(model, name='{}/p2_1x1'.format(name), bottom=bottom, num_output=p2_num_out,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
    model, top2a = addConvRelu(model, name='{}/p2_1x3'.format(name), bottom=top2, num_output=p2_num_out,
                                kernel_h=1, kernel_w=3, pad_h=0, pad_w=1, stride=1, filler="xavier")
    model, top2b = addConvRelu(model, name='{}/p2_3x1'.format(name), bottom=top2, num_output=p2_num_out,
    ...
    # p3:  1x1->3x3->([3x1] + [1x3])
    model, top3 = addConvRelu(model, name='{}/p3_1x1'.format(name), bottom=bottom, num_output=p3_proj,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
    model, top3 = addConvRelu(model, name='{}/p3_3x3'.format(name), bottom=top3, num_output=p3_num_out,
                                kernel_size=3, stride=1, pad=1, filler="xavier")
    model, top3a = addConvRelu(model, name='{}/p3_1x3'.format(name), bottom=top3, num_output=p3_num_out,
                                kernel_h=1, kernel_w=3, pad_h=0, pad_w=1, stride=1, filler="xavier")
    model, top3b = addConvRelu(model, name='{}/p3_3x1'.format(name), bottom=top3, num_output=p3_num_out,
                                kernel_h=3, kernel_w=1, pad_h=1, pad_w=0, stride=1, filler="xavier")
    p3_tops = [top3a, top3b]
...
def addAuxLoss(model, name, bottom):
    ...
    model, top = addConvRelu(model, name='{}/conv'.format(name), bottom=top, num_output=128,
                                kernel_size=1, stride=1, pad=0, filler="xavier")
...
def buildInception_v3():
    ...
    model,top = addConvRelu(model, 'conv1', data_top, num_output=32,
                              kernel_size=3, stride=2, pad=0, filler="xavier")
    # 149 x 149 x 32
    model,top = addConvRelu(model, 'conv2', top, num_output=32,
                              kernel_size=3, stride=1, pad=0, filler="xavier")
    # 147 x 147 x 32
    model,top = addConvRelu(model, 'conv3', top, num_output=64,
                              kernel_size=3, stride=1, pad=1, filler="xavier")
    # 147 x 147 x 64
    model,top = addPool(model, 'pool1', top, kernel_size=3, stride=2, pad=0, pool_type='MAX')
    # 73 x 73 x 64
    model,top = addConvRelu(model, 'conv4', top, num_output=80,
                              kernel_size=3, stride=1, pad=0, filler="xavier")
    # 73 x 73 x 80
    model,top = addConvRelu(model, 'conv5', top, num_output=192,
                              kernel_size=3, stride=2, pad=0, filler="xavier")
    # 71 x 71 x 192.
...
def main():
    ...
    fp = open("inception_v3_light.prototxt", 'w')
    fp.write(model)
