...
#------------------------------------------------------------------------------
def addResELU_large(model, name , bottom, num_output, group, j, fix_dim, dilation = False):

    prefix="{name}.{j}.".format(name=name,j=str(j))
    block=""
    block, top = addConvBnElu(model=block, name='{}conv1'.format(prefix), bottom=bottom, num_output=num_output,
                               kernel_size=1, group=1, stride=2 if (fix_dim and (j==1)) else 1, pad=0)
    block, top = addConvBnElu(model=block, name='{}conv2'.format(prefix), bottom=top, num_output=num_output,
    ...
    block, top = addActivation(model=block, name="{}elu".format(prefix), bottom=top, type="ELU")
...
def addResELU_SuperBlock(model, bottom, i, num_subBlocks, num_output, group, fix_dim, net_type, dilation = False):
    ...
    for j in xrange(1, num_subBlocks + 1):
        if net_type=="small":
          ...
          model, top = addResELU_large(model, name, bottom=top, num_output=num_output, group=group,
                                    j=j, fix_dim=fix_dim, dilation=dilation)
    return model, top
...
def buildResnetELU(netConfig, name, net_type):
    ...
    model, top = addConvBnElu(model, name="conv1", bottom="data", num_output=64,
...
def main():
    ...
    netConfig = numpy.matrix([
        ...
        [256, 6, 1, 1],
        [512, 3, 1, 1]])
    model = buildResnetELU(netConfig, name="Resnet50_ELU", net_type="large")
    fp = open("resnet50_elu.prototxt", 'w')
    fp.write(model)
