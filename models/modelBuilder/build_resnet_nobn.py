...
def addResNobn_small(model, name , bottom, num_output, group, j, fix_dim, dilation = False):
    ...
    block, top = addDropout(model=block, name='{}dropout'.format(prefix), bottom=top, ratio=0.2)
...
def buildResnetNobn(netConfig, name, net_type):
    ...
    for i in xrange(1, num_blocks+1):
        ...
        bottom=top
        model, top = addResNobn_SuperBlock(model, bottom, i+1, num_subBlocks, num_output, group, fix_dim, net_type)
