...
def addResSeluSuperBlock(model, bottom, i, num_subBlocks, num_output, group, fix_dim, net_type, dilation = False):
...
def buildResnetSeluModel(netConfig, name, net_type):
    ...
    for i in xrange(1, num_blocks+1):
        ...
        bottom=top
        model, top = addResSeluSuperBlock(model, bottom, i+1, num_subBlocks, num_output, group, fix_dim, net_type)
...
def main():
    ...
    model = buildResnetSeluModel(netConfig, name="Resnet18", net_type="small")
    fp = open("resnet18.prototxt", 'w')
    fp.write(model)
    ...
    netConfig = numpy.matrix([
        ...
        [256, 6, 1, 1],
        [512, 3, 1, 1]])
    model = buildResnetSeluModel(netConfig, name="Resnet34", net_type="small")
    fp = open("resnet34.prototxt", 'w')
    fp.write(model)
    ...
    netConfig = numpy.matrix([
        ...
        [256, 6, 1, 1],
        [512, 3, 1, 1]])
    model = buildResnetSeluModel(netConfig, name="Resnet50_Selu", net_type="large")
