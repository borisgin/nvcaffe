...
void CuDNNConvolutionLayer<Ftype, Btype>::LayerSetUp(
  ...
  createFilterDesc<Ftype>(&fwd_filter_desc_,
      this->num_output_ / fwd_group_factor(),
      this->channels_ / this->group_,
      kernel_h, kernel_w);
  createFilterDesc<Btype>(&bwd_filter_desc_,
      this->num_output_ / bwd_group_factor(),
      this->channels_ / this->group_,
      kernel_h, kernel_w);
...
void CuDNNConvolutionLayer<Ftype, Btype>::Reshape(
  ...
  for (int i = 0; i < bottom.size(); i++) {
    ...
    cudnn::setTensor4dDesc<Btype>(&bwd_bottom_descs_[i],
        this->num_,
        this->channels_ / bwd_group_factor(),
        height, width,
        this->channels_ * height * width,
        height * width, width, 1);
  ...
  if (use_algo_seeker_ && this->iterations_sized() <= 2) {
    ...
    if (use_modest_workspace_) {
      ...
      workspace_bytes = INITIAL_WORKSPACE_SIZE *  this->group_;
    } else {
  ...
  // Tensor descriptor for bias.
  if (this->bias_term_) {
    cudnn::setTensor4dDesc<Ftype>(&fwd_bias_desc_, 1, this->num_output_ / fwd_group_factor(), 1, 1);
    cudnn::setTensor4dDesc<Btype>(&bwd_bias_desc_, 1, this->num_output_ / bwd_group_factor(), 1, 1);
  }
}
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    if (user_algos_override_[1] < 0) {
      CUDNN_CHECK(cudnnGetConvolutionBackwardDataAlgorithm(Caffe::cudnn_handle(),
          ...
          align_down<7>(workspace_bytes), &bwd_data_algo_[i]));
    }
    ...
    if (user_algos_override_[0] < 0) {
      CUDNN_CHECK(cudnnGetConvolutionForwardAlgorithm(Caffe::cudnn_handle(),
          ...
          align_down<7>(workspace_bytes), &fwd_algo_[i]));
      CUDA_CHECK(cudaStreamSynchronize(Caffe::thread_stream()));
    }
    ...
    if (user_algos_override_[2] < 0) {
      CUDNN_CHECK(cudnnGetConvolutionBackwardFilterAlgorithm(Caffe::cudnn_handle(),
          ...
          align_down<7>(workspace_bytes), &bwd_filter_algo_[i]));
      CUDA_CHECK(cudaStreamSynchronize(Caffe::thread_stream()));
    }
        ...
        for (int k = 0; k < fwd_algo_count; ++k) {
          if (fwd_results[k].status == CUDNN_STATUS_SUCCESS) {
            ...
            mem_req_all_grps_ = std::max(mem_req_all_grps_,
                align_up<7>(workspace_fwd_sizes_[i]));
        ...
        for (int m = 0; m < 2; ++m) {
          ...
          for (int k = 0; k < filter_algo_count; ++k) {
            if (bwd_filter_results[k].status == CUDNN_STATUS_SUCCESS) {
              ...
              mem_req_all_grps_ = std::max(mem_req_all_grps_,
                  align_up<7>(workspace_bwd_filter_sizes_[i]));
...
bool CuDNNConvolutionLayer<Ftype, Btype>::IsBottomDescChanged(
  ...
  for (int i = 0; i < bottom.size(); i++) {
    ...
    n = shape[0];
    c = shape[1] / (fwd_mode ? fwd_groups() : bwd_groups());
    h = shape[2];
    w = shape[3];
...
void CuDNNConvolutionLayer<Ftype, Btype>::UpdateWorkspaceDemand(int size) {
  ...
  for (int i = 0; i < size; ++i) {
    req = align_up<7>(workspace_fwd_sizes_[i]);
    if (mem_req_all_grps_ < req) {
      mem_req_all_grps_ = req;
    }
    req = align_up<7>(workspace_bwd_data_sizes_[i]);
    if (mem_req_all_grps_ < req) {
      mem_req_all_grps_ = req;
    }
    req = align_up<7>(workspace_bwd_filter_sizes_[i]);
    if (mem_req_all_grps_ < req) {
      mem_req_all_grps_ = req;
    }
  }
