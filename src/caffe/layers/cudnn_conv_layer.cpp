...
#ifdef CUDNN_GROUPING
//    if (fwd_use_grouping()) {
      CUDNN_CHECK(cudnnSetConvolutionGroupCount(fwd_conv_desc, group_count()));//fwd_groups()));//this->group_));
//    }
#endif
#ifdef CUDNN_GROUPING
//    if (bwd_use_grouping()) {
      CUDNN_CHECK(cudnnSetConvolutionGroupCount(bwd_conv_data_desc, group_count()));//bwd_groups()));//this->group_));
      CUDNN_CHECK(cudnnSetConvolutionGroupCount(bwd_conv_filter_desc, group_count()));// bwd_groups()));//
//    }
#endif
...
#ifdef CUDNN_GROUPING
//    if (fwd_use_grouping()) {
      CUDNN_CHECK(cudnnSetConvolutionGroupCount(fwd_cached_conv_desc, group_count()));//fwd_groups()));
//    }
#endif
...
#ifdef CUDNN_GROUPING
//    if (bwd_use_grouping()) {
      CUDNN_CHECK(cudnnSetConvolutionGroupCount(bwd_cached_conv_data_desc, group_count()));//bwd_groups()));
      CUDNN_CHECK(cudnnSetConvolutionGroupCount(bwd_cached_conv_filter_desc, group_count()));//bwd_groups()));
//    }
#endif
...
void CuDNNConvolutionLayer<Ftype, Btype>::FindExConvAlgo(
  ...
  const size_t gsize = ws.size() / ngroups;
  CHECK(is_even(gsize)) << ws.size() << " / " << ngroups << " -> " << gsize;
  ...
  for (int i = 0; i < bottom.size(); ++i) {
        ...
        for (int k = 0; k < fwd_algo_count; ++k) {
          if (fwd_results[k].status == CUDNN_STATUS_SUCCESS) {
            ...
            mem_req_all_grps_ = std::max(mem_req_all_grps_,
                align_up<7>(workspace_fwd_sizes_[i] * groups()));
        ...
        for (int m = 0; m < 2; ++m) {
          ...
          for (int k = 0; k < filter_algo_count; ++k) {
            if (bwd_filter_results[k].status == CUDNN_STATUS_SUCCESS) {
              ...
              mem_req_all_grps_ = std::max(mem_req_all_grps_,
                  align_up<7>(workspace_bwd_filter_sizes_[i] * groups()));
        ...
        for (int m = 0; m < 2; ++m) {
          ...
          for (int k = 0; k < data_algo_count; ++k) {
            if (bwd_data_results[k].status == CUDNN_STATUS_SUCCESS) {
              ...
              mem_req_all_grps_ = std::max(mem_req_all_grps_,
                  align_up<7>(workspace_bwd_data_sizes_[i] * groups()));
    ...
    os << this->print_current_device()
        ...
        << gb_round2(ws.size()) << "G " //<< group_count() // << "/" << bwd_groups()
        << "/" << this->channels_
#ifdef DEBUG
